...
class ComputeLoss_val:
    ...
    def __init__(self, model, autobalance=False):
        super(ComputeLoss_val, self).__init__()
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        # Losses
        for i, pi in enumerate(p):  # layer index 3, layer predictions with different grid choice
...
class ComputeLoss:
    ...
    def build_targets_v1(self, p, targets):
        ...
        na, nt = self.na, targets.shape[0]  # number of anchors = 3, targets = 201600
        tcls, tbox, indices, anch = [], [], [], []
        num_entry = targets.shape[1] #aim: do not change last three entries out of 8
        gain = torch.ones(num_entry+1, device=targets.device)  # normalized to gridspace gain
