...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    cstargets_bcc = torch.tensor(cstargets_all_bcc['train']).to(device) if torchMode else cstargets_all_bcc['train']
    ...
    pcm = {k: torch.tensor(v).to(device) if torchMode else v for k, v in compute_param_confusion_matrices(bcc_params).items()}
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss, loss_items = compute_loss(pred, qtargets_yolo)
                if RANK != -1:
...
if __name__ == "__main__":
    ...
    opt.cache = None
    opt.workers = 0
    opt.batch_size = 150 # Change this to number of train images
    opt.epochs = 150
    #opt.device = 'cpu'
    main(opt)
