    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                loss1, loss_items1 = compute_loss(pred1, torch.cat(batch_cstargets_union).to(device))
            ...
            if ni1 - last_opt_step1 >= accumulate1:
                scaler1.step(optimizer1)  # optimizer.step
                scaler1.update()
            ...
            if RANK in [-1, 0]:
                ...
                mloss1 = (mloss1 * i + loss_items1) / (i + 1)  # update mean losses
                mem1 = f'{torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0:.3g}G'  # (GB)
                pbar.set_description(('%10s' * 2 + '%10.4g' * 5) % (
                    f'{epoch}/{epochs - 1}', mem1, *mloss1, targets.shape[0], imgs.shape[-1]))
                callbacks.on_train_batch_end(ni1, model1, imgs, targets, paths1, plots1, opt.sync_bn)
            # end batch ------------------------------------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            ema1.update_attr(model1, include=['yaml', 'nc', 'hyp', 'names', 'stride', 'class_weights'])
            ...
            if not noval or final_epoch:  # Calculate mAP
                ...
                results1, maps1, _ = val.run(data_dict,
                                           batch_size=batch_size // WORLD_SIZE * 2,
                                           imgsz=imgsz,
                                           model=ema1.ema,
                                           single_cls=single_cls,
